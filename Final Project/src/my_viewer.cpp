...
void MyViewer::build_scene ()
{
	...
	// Dial
	float R = dialR;
	/*p = new SnPrimitive(GsPrimitive::Box, R, R, R/10.0f);*/
	p = new SnPrimitive(GsPrimitive::Box, R/2.0f, R, R/2.0f);
...
void MyViewer::run_animation ()
{
	...
	/*if (camView == true) {
		...
		do {
			...
			if (lt > 2.0f && lt <= 5.0f) {
				camera().eye.x -= 0.0010f;
				camera().eye.z -= 0.0010f;
			}
			if (lt > 5.0f && lt <= 10.0f) {
				camera().eye.x -= 0.0020f;
				camera().eye.z -= 0.0020f;
			}
			if (lt > 10.0f && lt <= 15.0f) {
				//camera().eye.x -= 0.0010f;
				camera().eye.z -= 0.0020f;
				camera().eye.y += 0.0010f;
			}
			render();
			ws_check();
